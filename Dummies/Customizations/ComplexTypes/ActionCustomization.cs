...
public sealed class ActionCustomization : CustomizationBase
{
    ...
    private static readonly MethodInfo[] CreateMethodsByArity = typeof(ActionCustomization)
        .GetMethods(BindingFlags.NonPublic | BindingFlags.Static)
        .Where(m => m is { Name: nameof(Create), IsGenericMethodDefinition: true })
        .OrderBy(m => m.GetGenericArguments().Length)
        .ToArray();

    private static readonly ConcurrentDictionary<TypeArrayKey, MethodInfo> Cache = new();
    ...
    protected override IDummyBuilder BuildMe(IDummy dummy, Type type)
    {
        return dummy.Build<object>().FromFactory(() =>
        {
            ...
            var method = Cache.GetOrAdd(new TypeArrayKey(genericArguments), key =>
    ...
    {
        var id = Guid.NewGuid();
        return (_, _, _, _, _, _, _, _, _, _, _, _, _, _) => { _ = id; };
    }
    ...
    {
        var id = Guid.NewGuid();
        return (_, _, _, _, _, _, _, _, _, _, _, _, _, _, _) => { _ = id; };
    }
    ...
    {
        var id = Guid.NewGuid();
        return (_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _) => { _ = id; };
    }
}
